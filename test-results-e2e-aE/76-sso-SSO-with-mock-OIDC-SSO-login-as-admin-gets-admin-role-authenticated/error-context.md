...
- Name: 76-sso.spec.ts >> SSO with mock OIDC >> SSO login as admin gets admin role
- Location: test/e2e/76-sso.spec.ts:47:3
...
Expected pattern: /localhost:3004\/dex/
Received string:  "http://localhost:3040/api/auth/sso/login"
Timeout: 5000ms

Call log:
  - Expect "toHaveURL" with timeout 5000ms
    14 × unexpected value "http://localhost:3040/api/auth/sso/login"

```

```yaml
- text: "{\"error\":\"Failed to initiate SSO login\"}"
```
