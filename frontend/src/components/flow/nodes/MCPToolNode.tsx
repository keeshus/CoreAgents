  ...
  return (
    <BaseNode label={(props.data?.label as string) || 'MCP Tool'} nodeType="MCP Tool" category="tools" selected={props.selected || false} inputs={0} outputs={0} toolOutput warnings={props.data?._warnings as string[] | undefined}>
      <div className="space-y-1">
        <p className="truncate"><span className="text-on-surface-variant">Server:</span> {config?.serverName || 'Not set'}</p>
        <p className="truncate"><span className="text-on-surface-variant">Tool:</span> {config?.toolName || 'Not set'}</p>
