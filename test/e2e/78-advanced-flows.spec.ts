  ...
  test('debug: LLM, code, branch, feedback HITL loop, subflow, second HITL, output', async ({ page, request }) => {
    ...
    const flowDef = {
      ...
      nodes: [
        ...
        { id: 'b1', type: 'branch', position: { x: 600, y: 0 }, data: { label: 'Route', type: 'branch', config: { condition: 'input.prep.decision === "approve"' } } },
  ...
  test('persisted: dual HITL approvals with subflow enrichment', async ({ page, request }) => {
    ...
    const res = await createFlow(request, {
      ...
      nodes: [
        ...
        { id: 'b1', type: 'branch', position: { x: 600, y: 0 }, data: { label: 'Route', type: 'branch', config: { condition: 'input.check.decision === "approve"' } } },
