...
test.describe('All node types', () => {
  ...
  test('branch node routes based on condition', async ({ request }) => {
    ...
    const res = await createFlow(request, {
      ...
      nodes: [
        { id: 't1', type: 'trigger', position: { x: 0, y: 0 }, data: { label: 'Trigger', type: 'trigger', config: { triggerType: 'manual' } } },
        { id: 'b1', type: 'branch', position: { x: 300, y: 0 }, data: { label: 'Check', type: 'branch', config: { condition: 'input.message === "yes"' } } },
  ...
  test('branch false path routes correctly', async ({ request }) => {
    const name = uniqueFlowName('BranchFalseTest');
    const res = await createFlow(request, {
      name,
      nodes: [
        { id: 't1', type: 'trigger', position: { x: 0, y: 0 }, data: { label: 'Trigger', type: 'trigger', config: { triggerType: 'manual' } } },
        { id: 'b1', type: 'branch', position: { x: 300, y: 0 }, data: { label: 'Check', type: 'branch', config: { condition: 'input.message === "no"' } } },
        { id: 'o1', type: 'output', position: { x: 600, y: -100 }, data: { label: 'TruePath', type: 'output', config: { inputFields: ['check.verdict'] } } },
        { id: 'o2', type: 'output', position: { x: 600, y: 100 }, data: { label: 'FalsePath', type: 'output', config: { inputFields: ['check.verdict'] } } },
      ],
      edges: [
    ...
    expect(branchStep).toBeDefined();
    expect(branchStep!.data?.output?.verdict).toBe(true);
    // Now test false path — "yes" !== "no" → verdict=false, label='false', handle-0 routes to o1
    ...
    expect(branchStep2!.data?.output?.verdict).toBe(false);
