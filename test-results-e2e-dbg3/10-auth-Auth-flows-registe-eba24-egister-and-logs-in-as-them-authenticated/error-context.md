...
```ts
  ...
  65  | 
  66  |   test('registers a new user via /register and logs in as them', async ({ page, request, playwright }) => {
  ...
  98  |     const adminCtx = await playwright.request.newContext({ storageState: getAdminAuthFile() });
  99  |     try {
